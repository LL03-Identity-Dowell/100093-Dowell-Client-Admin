...
def fetch_data(user):
    ...
    login = dowellconnection("login","bangalore","login","os","os","1108","ABCDE","fetch",field_c,"nil")
...
@api_view(['POST'])
def solutions_func(request):
    try:
        field_c = {}
        
        field_c['user'] = request.data['user']
        field_c['process_name']= request.data['process_name']
        field_c['process_id'] = request.data['process_id']
        field_c['prodecessor'] = request.data['prodecessor']
        field_c['product'] = request.data['product']
        field_c['portfolio'] = request.data['portfolio']
        field_c['team_member'] = request.data['team_member']
        field_c['public'] = request.data['public']
        field_c['successor'] = request.data['successor']
        field_c['note'] = request.data['note']
        solution_data = request.data
        # print('Solutions data ', type(request.data['user']))
        
        xx = fetch_data(request.data['user'])
        res_data = xx['data']
        data_len =  len(res_data)
        
        if data_len >= 1:
            update_field = field_c
            field_c = {}
            # print('this is update field ', update_field)
            # print('this is field_c ', field_c)
            
            login = dowellconnection("login","bangalore","login","os","os","1108","ABCDE","update",field_c, update_field)
            resp = json.loads(login)
        
        elif data_len == 0:
            login = dowellconnection("login","bangalore","login","os","os","1108","ABCDE","insert",field_c, "nil")
            resp = json.loads(login)
