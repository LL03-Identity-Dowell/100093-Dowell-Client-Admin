...
const Form3 = () => {
    ...
    try {
      await axios
        ...
        .then((res) => {
          ...
          const updatedpendingmember = team_member.pending_members.filter(
  ...
  const query = team_member?.pending_members.map((option) => ({
  ...
  const handleSearchInputChange = (query: Option | null) => {
    if (query) {
      const filtermmember = team_member?.pending_members.find(
          ...
          disabled={teamMemberAccess === "View"}
            className={`w-full ${
              color_scheme == "Red"
                ...
                ? "bg-[#14A44D]"
                : "bg-[#7A7A7A]"
            }  hover:bg-[#61CE70] text-white  py-2 px-4 mb-8 rounded-md`}
          >
