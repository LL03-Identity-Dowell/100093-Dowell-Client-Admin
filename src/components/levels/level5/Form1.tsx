...
const Form1 = () => {
  ...
  let getLevelName = useSelector(
    (state: RootState) =>
      state.adminData.data[0]?.organisations[0]?.level5?.level_name
  );

  let getLevelItemLength = useSelector(
    (state: RootState) =>
      state.adminData.data[0]?.organisations[0]?.level5?.items.length
  );
  ...
  if (isnewOwner) {
    getLevelName = useSelector(
      (state: RootState) =>
        state.adminData.data[0]?.organisations[1]?.level5?.level_name
    );
    ...
    getLevelItemLength = useSelector(
      (state: RootState) =>
        state.adminData.data[0]?.organisations[1]?.level5?.items.length
    );
  }
  ...
  return (
		<>
			...
			<div className="lg:w-1/3 border border-[#54595F] card-shadow">
				...
				>
					<p id="level5headingtext">{`Level 5 – <${getLevelName}>,`}</p>
					<p id="level5headingtext_counter">{`${getLevelItemLength} – <total enabled items in level 5>`}</p>
				</span>

				<form className="px-[30px] mb-8" onSubmit={handleSubmit}>
					<div className="mb-4">
						<label
							id="level5_subheading_1"
