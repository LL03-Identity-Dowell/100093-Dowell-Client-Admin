...
const Form1 = () => {
  ...
  if (isnewOwner) {
    getLevelName = useSelector(
      (state: RootState) =>
        state.adminData.data[0]?.organisations[1]?.level2?.level_name
    );
    ...
    getLevelItemLength = useSelector(
      (state: RootState) =>
        state.adminData.data[0]?.organisations[1]?.level2?.items.length
    );
  }
  ...
  return (
		<>
			...
			<div className="lg:w-1/3 border border-[#54595F] card-shadow">
				...
				>
					<p id="level2headingtext">{`Level 2 – <${getLevelName}>,`}</p>
					<p id="level2headingtext_counter">{`${getLevelItemLength} – <total enabled items in level 2>`}</p>
				</span>

				<form className="px-[30px] mb-8" onSubmit={handleSubmit}>
					<div className="mb-4">
						<label
							id="level2_subheading_1"
