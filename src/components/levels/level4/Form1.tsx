...
const Form1 = () => {
  ...
  let getLevelName = useSelector(
    (state: RootState) =>
      state.adminData.data[0]?.organisations[0]?.level4?.level_name
  );

  let getLevelItemLength = useSelector(
    (state: RootState) =>
      state.adminData.data[0]?.organisations[0]?.level4?.items.length
  );
  ...
  if (isnewOwner) {
    getLevelName = useSelector(
      (state: RootState) =>
        state.adminData.data[0]?.organisations[1]?.level4?.level_name
    );
    ...
    getLevelItemLength = useSelector(
      (state: RootState) =>
        state.adminData.data[0]?.organisations[1]?.level4?.items.length
    );
  }
  ...
  return (
		<>
			...
			<div className="lg:w-1/3 border border-[#54595F] card-shadow">
				...
				>
					<p id="level4headingtext">{`Level 4 – <${getLevelName}>,`}</p>
					<p id="level4headingtext_counter">{`${getLevelItemLength} – <total enabled items in level 4>`}</p>
				</span>

				<form className="px-[30px] mb-8" onSubmit={handleSubmit}>
					<div className="mb-4">
						<label
							id="level4_subheading_1"
