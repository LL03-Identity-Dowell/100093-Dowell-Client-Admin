...
const Form1 = () => {
  ...
  let getLevelName = useSelector(
    (state: RootState) =>
      state.adminData.data[0]?.organisations[0]?.level3?.level_name
  );

  let getLevelItemLength = useSelector(
    (state: RootState) =>
      state.adminData.data[0]?.organisations[0]?.level3?.items.length
  );
  ...
  if (isnewOwner) {
    getLevelName = useSelector(
      (state: RootState) =>
        state.adminData.data[0]?.organisations[1]?.level3?.level_name
    );
    ...
    getLevelItemLength = useSelector(
      (state: RootState) =>
        state.adminData.data[0]?.organisations[1]?.level3?.items.length
    );
  }
                ...
                organisations: [
                  {
                    ...
                    level3: {
                      ...currentadmindata.data[0].organisations[0].level3,
                      level_name: levelName,
                    },
                  },
                ],
  ...
  return (
    <>
      ...
      <div className="lg:w-1/3 border border-[#54595F] card-shadow">
        ...
        >
          <p id="level3headingtext">{`Level 3 – <${getLevelName}>,`}</p>
          <p id="level3headingtext_counter">{`${getLevelItemLength} – <total enabled items in level 3>`}</p>
        </span>

        <form className="px-[30px] mb-8" onSubmit={handleSubmit}>
          <div className="mb-4">
            <label
              id="level3_subheading_1"
